"""
Phase Estimation Benchmark Program - Fire Opal
"""
...
def PhaseEstimation(num_qubits, theta):
    ...
    qc.measure(
        [qr[m] for m in range(num_counting_qubits)], list(range(num_counting_qubits))
    )
...
def analyze_and_print_result(qc, result, num_counting_qubits, theta):
    ...
    if verbose:
        ...
        print(
            f"For theta {theta}, app expected: {app_correct_dist} measured: {app_counts}"
        )
...
def theta_to_bitstring(theta, num_counting_qubits):
    counts = {
        format(
            int(theta * (2**num_counting_qubits)),
            "0" + str(num_counting_qubits) + "b",
        ): 1.0
    }
    return counts
...
def run(
    ...
    print("Phase Estimation Benchmark Program - Fire Opal")

    num_state_qubits = (
        1  # default, not exposed to users, cannot be changed in current implementation
    )

    # validate parameters (smallest circuit is 3 qubits)
    num_state_qubits = max(1, num_state_qubits)
    if max_qubits < num_state_qubits + 2:
        raise ValueError(f"ERROR: PE Benchmark needs at least {num_state_qubits + 2} qubits to run")
    ...
    def execution_handler(qc, result, num_qubits, theta):
        ...
        _, fidelity = analyze_and_print_result(
            qc, result, num_counting_qubits, float(theta)
        )
    ...
    for num_qubits in range(min_qubits, max_qubits + 1):
        ...
        if 2 ** (num_counting_qubits) <= max_circuits:
            theta_range = [
                i / (2 ** (num_counting_qubits)) for i in list(range(num_circuits))
            ]
        else:
            theta_range = [
                i / (2 ** (num_counting_qubits))
                for i in np.random.choice(
                    2 ** (num_counting_qubits), num_circuits, False
                )
            ]
        ...
        for theta in theta_range:
            ...
            # collapse the 3 sub-circuit levels used in this benchmark
    ...
    print("\nPhase Operator 'U' = ")
    print(U_ if U_ != None else "  ... too large!")
    print("\nInverse QFT Circuit =")
    print(QFTI_ if QFTI_ != None else "  ... too large!")

    # Plot metrics for all circuit sizes
    metrics.plot_metrics("Benchmark Results - Phase Estimation - Fire Opal")
