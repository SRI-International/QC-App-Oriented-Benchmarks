"""
Shor's Order Finding Algorithm Benchmark - Fire Opal
"""
...
sys.path[1:1] = [
    ...
    "_common/fireopal",
    "shors/_common",
    "quantum-fourier-transform/fireopal",
]
sys.path[1:1] = [
    "../../_common",
    "../../_common/fireopal",
...
from fire_utils import remove_multiple_registers
...
def phiADD(num_qubits, a):
    ...
    global PHIADD_
    if PHIADD_ is None or num_qubits <= 3:
        if num_qubits < 4:
            PHIADD_ = qc
...
# Circuit that implements doubly controlled modular addition by a (num qubits should be
# bit count for number N)
def ccphiADDmodN(num_qubits, a, N):
    ...
    ctl_main_qubits = list(qr_ctl)
    ctl_main_qubits.extend(list(qr_main))
    anc_main_qubits = [qr_ancilla[0]]
    anc_main_qubits.extend(list(qr_main))
    ...
    if CCPHIADDMODN_ is None or num_qubits <= 2:
        if num_qubits < 3:
            CCPHIADDMODN_ = qc
...
# Creates circuit that implements single controlled modular multiplication by a. n
# represents the number of bits needed to represent the integer number N
...
def controlled_Ua(n, a, exponent, N):
    ...
    # Create relevant temporary qubit list
    qubits = list(qr_ctl)
    qubits.extend(list(qr_x))
    qubits.extend(list(qr_main))
    qubits.extend(list(qr_ancilla))
...
def ShorsAlgorithm(number, base, method, verbose=verbose):
    ...
    if method == 1:
        ...
        for i in reversed(range(2 * n)):
            ...
            # Create relevant temporary qubit list
            qubits = [qr_counting[i]]
            qubits.extend(list(qr_mult))
            qubits.extend(list(qr_aux))
        ...
        cr_aux = ClassicalRegister(
            1
        )  # Register to reset the state of the up register based on previous measurements
    ...
    global QC_, QFT_
    if QC_ is None or n <= 2:
        if n < 3:
            QC_ = qc
    if QFT_ is None or n <= 2:
        if n < 3:
            QFT_ = qft_gate(n + 1)
...
def analyze_and_print_result(qc, result, num_qubits, order, method):
...
def run(
    ...
    method=1,
    verbose=verbose,
    backend_id="fire_opal_ibmq_guadalupe",
    device_credentials=None,
):

    print("Shor's Order Finding Algorithm Benchmark - Fire Opal")
    print(f"... using circuit method {method}")

    # Each method has a different minimum amount of qubits to run and a certain multiple
    # of qubits that can be run
    ...
    # number_order contains an array of length 2 with the number and order
    def execution_handler(qc, result, num_qubits, number_order):
        # determine fidelity of result set
        num_qubits = int(num_qubits)
        # Must convert number_order from string to array
        order = eval(number_order)[1]
        _, fidelity = analyze_and_print_result(qc, result, num_qubits, order, method)
    ...
    for num_qubits in range(min_qubits, max_qubits + 1, qubit_multiple):
        ...
        for _ in range(num_circuits):
            ...
            # Checking if generated order can be reduced. Can also run through prime list
            # in shors utils
            if order % 2 == 0:
                order = 2
            if order % 3 == 0:
            ...
            metrics.store_metric(
                num_qubits, number_order, "create_time", time.time() - ts
            )

            # collapse the 4 sub-circuit levels used in this benchmark (for Fire Opal)
            qc = qc.decompose().decompose().decompose().decompose()
            qc2 = remove_multiple_registers(qc)

            # submit circuit for execution on target (simulator, cloud simulator, or hardware)
            ex.submit_circuit(qc2, num_qubits, number_order, num_shots)

        # execute all circuits in batch, aggregate and report metrics when complete
        ex.execute_circuits()
    ...
    print(CUA_ if CUA_ is not None else " ... too large!")
    print("\nControlled Multiplier Operator 'cMULTamodN' =")
    print(CMULTAMODN_ if CMULTAMODN_ is not None else " ... too large!")
    print("\nControlled Modular Adder Operator 'ccphiamodN' =")
    print(CCPHIADDMODN_ if CCPHIADDMODN_ is not None else " ... too large!")
    print("\nPhi Adder Operator '\u03C6ADD' =")
    print(PHIADD_ if PHIADD_ is not None else " ... too large!")
    print("\nQFT Circuit =")
    print(QFT_ if QFT_ is not None else "  ... too large!")

    # Plot metrics for all circuit sizes
    metrics.plot_metrics(
        f"Benchmark Results - Shor's Order Finding ({method}) - Fire Opal"
    )
