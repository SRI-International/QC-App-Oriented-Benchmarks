...
- **Qiskit parallel features** — Topology+error-aware partition mapping selects low-error qubit regions on the device. Mixed-width circuit support packs circuits of different widths into available partitions. Array batching submits all circuits as one job via Qiskit `ParallelExperiment`. High-error partitions are filtered (default threshold 2% avg gate error). **Requires additional packages**: `pip install qiskit-experiments networkx` (not included in base install since Qiskit itself is optional).
- **Timing decomposition** — The execute module now exposes `last_transpile_time`, `last_exec_time`, and `last_elapsed_time` after each `execute_circuits()` call, enabling callers to report accurate timing breakdowns. Quantum execution time is extracted from hardware result metadata (execution_spans) when available, giving the true QPU time independent of queue wait and transpilation overhead. The Hamlib observable benchmark reports all three: total elapsed, quantum execution time, and transpile/overhead time.
