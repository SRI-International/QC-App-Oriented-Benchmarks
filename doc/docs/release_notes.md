...
- **Qiskit parallel features** — Topology+error-aware partition mapping selects low-error qubit regions on the device. Mixed-width circuit support packs circuits of different widths into available partitions. Array batching submits all circuits as one job via Qiskit `ParallelExperiment`. High-error partitions are filtered (default threshold 2% avg gate error). Tested on ibm_fez with 2-3x cost reduction per circuit. **Requires additional packages**: `pip install qiskit-experiments networkx` (not included in base install since Qiskit itself is optional). Currently tested on IBM hardware only; may work on other Qiskit-compatible backends.
- **Removed deprecated execution pipeline** — The old one-circuit-at-a-time `submit_circuit`/`throttle_execution`/`finalize_execution` pipeline has been removed (~810 lines). All Qiskit benchmarks now use the array-based `submit_circuits`/`execute_circuits` path. QRL benchmark converted; unused benchmark variants (hhl_benchmark_km, hamlib_v1, WIP tfim/mbl) removed.
- **Improved benchmark timing accuracy** — Moved the Qiskit transpiler warmup from `execute_circuits()` to `set_execution_target()`, so the ~4s one-time transpiler initialization cost is no longer included in benchmark execution timing. Also improved the warmup circuit to fully prime Aer's batch execution path, eliminating a ~2ms first-circuit timing artifact. Parallel execution extracts real QPU timing from IBM job results (`execution_spans`) for accurate per-circuit cost reporting.
