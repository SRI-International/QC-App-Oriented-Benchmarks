"""
Quantum Fourier Transform Benchmark Program - Fire Opal
"""
...
def QuantumFourierTransform (num_qubits, secret_int, method=1):
    ...
    # Input uses all available qubits
...
def analyze_and_print_result (qc, result, num_qubits, secret_int, method):
    ...
    if verbose:
        print(f"For secret int {secret_int} measured: {counts} fidelity: {fidelity}")
...
def run(
    ...
    print("Quantum Fourier Transform Benchmark Program - Fire Opal")
    ...
    metrics.plot_metrics(f"Benchmark Results - Quantum Fourier Transform ({method}) - Fire Opal")
