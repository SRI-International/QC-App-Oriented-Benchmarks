...
#
#################################
# Image-Recognition Metrics Module
#
# This module contains methods to initialize, store, aggregate and
# plot metrics collected in the benchmark programs for the Image-Recognition
#
...
def find_metrics_array_for_group(group, instance, metric_name, x_val, x_metric_name):   
    ...
    for circuit_id in image_recognition_metrics[group]:
        if np.floor(int(circuit_id)/1000) == instance:
            ...
            if metric_name in image_recognition_metrics[group][circuit_id]:
                metric_value = image_recognition_metrics[group][circuit_id][metric_name]
            else:
                metric_value = 0
            ...
            x_metric_value = image_recognition_metrics[group][circuit_id][x_metric_name]
            # set the x label
...
def plot_all_line_metrics(suptitle=None, options=None,
    ...
    igroup_keys = sorted([int(key) for key in image_recognition_metrics.keys()])
    ...
    for qubit_count in group_keys:
        ...
        circuit_ids = [int(x) for x in (image_recognition_metrics[qubit_count])]
        ...
        for instance in range(1, total_instances + 1):
            ...
            suptitle = toptitle + f"\nqubits={num_qubits}, shots={options['shots']}, ansatz={ansatz_type}, method={method}"
            ...
            for jj in range(plot_count):
                ...
                if save_plot_images:
                    image_name = (f"Image-Recognition-({method})-line-{group}-{instance}") + \
...
def plot_all_cumulative_metrics(suptitle=None,
    ...
    for num_qubits in image_recognition_metrics:
        group = str(num_qubits)
        
        circuit_ids = [int(x) for x in (image_recognition_metrics[group])]
        total_instances = int(np.floor(circuit_ids[-1]/1000))
...
def plot_cumulative_metrics(suptitle="",
    ...
    # save the plot image
    if save_plot_images:
        metrics.save_plot_image(plt, os.path.join(f"Image-Recognition-(2)" +
                                            "-" + suffix),
                                            backend_id)
    # show the plot(s)
    plt.show(block=True)
