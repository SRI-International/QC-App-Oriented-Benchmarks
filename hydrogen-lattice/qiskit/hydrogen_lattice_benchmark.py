...
def VQE_ansatz(
    num_qubits: int, thetas_array, parameterized, num_occ_pairs: Optional[int] = None, *args, **kwargs
) -> QuantumCircuit:
    if verbose:
...
def HydrogenLattice(num_qubits, operator, secret_int=000000, thetas_array=None, parameterized=None):
    ...
    if ex.do_transpile_for_execute:
        ...
        _qc, parameter_vector, thetas_array = VQE_ansatz(
            num_qubits=num_qubits, thetas_array=thetas_array, parameterized=parameterized, num_occ_pairs=None
        )
...
def run(
        ...
        for instance_num in range(1, max_circuits + 1):
            ...
            if method == 2:
                ...
                def objective_function(thetas_array):
                    ...
                    if verbose:
                        print(
                            f"... ** compute energy for num_qubits={num_qubits}, circuit={unique_id}, parameters={params}, thetas_array={thetas_array}"
                        )
    ...
    if method == 1:
        metrics.plot_metrics(f"Benchmark Results - Hydrogen Lattice ({method}) - Qiskit", options=dict(shots=num_shots))
