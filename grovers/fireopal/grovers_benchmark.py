"""
Grover's Search Benchmark Program - Fire Opal
"""
...
def analyze_and_print_result(qc, result, num_qubits, marked_item):
    ...
    if verbose: 
        print(f"For type {marked_item} measured: {counts}")

    # we compare counts to analytical correct distribution
    correct_dist = grovers_dist(num_qubits, marked_item)
    if verbose: 
        print(f"Marked item: {marked_item}, Correct dist: {correct_dist}")
...
def run(
    ...
    print("Grover's Search Benchmark Program - Fire Opal")
    ...
    print("\nOracle =")
    print(grover_oracle if grover_oracle is not None else "  ... too large!")
    print("\nDiffuser =")
    print(diffusion_operator)

    # Plot metrics for all circuit sizes
    metrics.plot_metrics("Benchmark Results - Grover's Search - Fire Opal")
