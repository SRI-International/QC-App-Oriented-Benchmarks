...
Tests on local qasm_simulator — no external backend needed.

Run from benchmark root:
    python _tests/test_execute_circuits.py
"""
...
from qedclib.qiskit import execute as ex
...
def test_execute_circuits_basic():
    ...
    circuits = [create_bell_circuit() for _ in range(3)]
...
def test_execute_circuits_single():
    ...
    circuits = [create_bell_circuit()]
    job_id, results = ex.execute_circuits(circuits, num_shots=100)

    counts_list = results.get_counts()
    # Single circuit — ExecutionResult may unwrap to dict
    if isinstance(counts_list, dict):
        counts_list = [counts_list]
...
def test_process_circuit_results():
    ...
    # Set up execution and result handler
...
def test_submit_circuits_batch_by_group():
    ...
    # Check that groups were processed separately (first 3 calls should be group "2")
    for i in range(3):
...
if __name__ == '__main__':
    print("Testing new execute_circuits / process_circuit_results / submit_circuits")
