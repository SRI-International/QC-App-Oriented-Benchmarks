...
def get_circuits(
    ...
    """Create Bernstein-Vazirani benchmark circuits.
    ...
    Standard args (common to all benchmarks):
        ...
        method: algorithm method; 2 = mid-circuit measurement (default 1)
    ...
    kernel = qedc_get_kernel("bv_kernel", api=api)
    ...
    # Track qubit group sizes for mid-circuit measurement plotting
    global mid_circuit_qubit_group
    ...
    for num_qubits in range(min_qubits, max_qubits + 1, skip_qubits):
        ...
        for s_int in s_range:
            ...
            # for mid-circuit measurement, track that each circuit uses only 2 qubits
            if method == 2:
...
def analyze_and_print_result(qc, result, num_qubits, num_shots, s_int=None):
    ...
    # Expected result is the secret_int measured with 100% probability
...
def run_circuits(all_qcs,
    ...
    """
    qedc_get_kernel("bv_kernel", api=api)
...
def plot_results(
    ...
    # for mid-circuit measurement, remap qubit groups in the plot
    ...
    if qedc_is_leader():
        ...
        if plot_results:
            ...
            metrics.plot_metrics(
                ...
                options=options,
                transform_qubit_group=transform_qubit_group,
...
import argparse
def get_args():
    parser = argparse.ArgumentParser(description="Bernstein-Vazirani Benchmark")
