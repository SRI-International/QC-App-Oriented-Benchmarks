"""
Amplitude Estimation Benchmark Program via Phase Estimation - Fire Opal
"""
...
sys.path[1:1] = ["_common", "_common/fireopal", "quantum-fourier-transform/fireopal"]
sys.path[1:1] = [
    "../../_common",
    "../../_common/fireopal",
...
def AmplitudeEstimation(
    num_state_qubits, num_counting_qubits, a, psi_zero=None, psi_one=None
):
    ...
    if (cQ_ and Q_) is None or num_state_qubits <= 6:
        if num_state_qubits < 9:
            cQ_ = cQ
            Q_ = Q
            A_ = A
    if QFTI_ is None or num_qubits <= 5:
        if num_qubits < 9:
            QFTI_ = inv_qft_gate(num_counting_qubits)
    ...
    for j in reversed(range(num_counting_qubits)):
        for _ in range(repeat):
            qc.append(
                cQ,
                [qr_counting[j]] + [qr_state[l] for l in range(num_state_qubits + 1)],
            )
        repeat *= 2
    ...
    qc.measure(
        [qr_counting[m] for m in range(num_counting_qubits)],
        list(range(num_counting_qubits)),
    )
...
def Ctrl_Q(num_state_qubits, A_circ):
    ...
    # A_circ_inverse
    qc.append(A_gate_inv, list(range(num_state_qubits + 1)))
    ...
    # A_circ
    qc.append(A_gate, list(range(num_state_qubits + 1)))
...
def analyze_and_print_result(qc, result, num_counting_qubits, s_int):
    ...
    if verbose:
        ...
        print(
            f"For amplitude {a}, app expected: {app_correct_dist} measured: {app_counts}"
        )
...
def a_to_bitstring(a, num_counting_qubits):
    ...
    if num1 != num2 and num2 < 2**m and num1 < 2**m:
        counts = {
            format(num1, "0" + str(m) + "b"): 0.5,
            format(num2, "0" + str(m) + "b"): 0.5,
        }
    else:
...
def run(
    ...
    print("Amplitude Estimation Benchmark Program - Fire Opal")

    # Clamp the maximum number of qubits
    if max_qubits > MAX_QUBITS:
        print(
            f"INFO: Amplitude Estimation benchmark is limited to a maximum of {MAX_QUBITS} qubits."
        )
    ...
    if max_qubits < num_state_qubits + 2:
        print(
            f"ERROR: AE Benchmark needs at least {num_state_qubits + 2} qubits to run"
        )
        return
    ...
    def execution_handler(qc, result, num_qubits, s_int):
        ...
        _, fidelity = analyze_and_print_result(
            qc, result, num_counting_qubits, int(s_int)
        )
    ...
    for num_qubits in range(min_qubits, max_qubits + 1):
        ...
        if verbose:
            print(
                f"              with num_state_qubits = {num_state_qubits}  num_counting_qubits = {num_counting_qubits}"
            )
        ...
        for s_int in s_range:
            ...
            # collapse the 3 sub-circuit levels used in this benchmark (for Fire Opal)
            qc2 = qc.decompose().decompose().decompose()
            qc3 = remove_multiple_registers(qc2)

            # submit circuit for execution on target (simulator, cloud simulator, or hardware)
            ex.submit_circuit(qc3, num_qubits, s_int, num_shots)

        # execute all batched circuits, aggregate and report metrics when complete
        ex.execute_circuits()
    ...
    print(cQ_ if cQ_ is not None else " ... too large!")
    print("\nQuantum Operator 'Q' =")
    print(Q_ if Q_ is not None else " ... too large!")
    print("\nAmplitude Generator 'A' =")
    print(A_ if A_ is not None else " ... too large!")
    print("\nInverse QFT Circuit =")
    print(QFTI_ if QC_ is not None else "  ... too large!")

    # Plot metrics for all circuit sizes
    metrics.plot_metrics("Benchmark Results - Amplitude Estimation - Fire Opal")
