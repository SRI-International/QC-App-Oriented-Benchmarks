'''
Hidden Shift Benchmark Program - v2 Architecture Prototype
...
def get_circuits(
    ...
    min_qubits=2, max_qubits=6, skip_qubits=2,
    max_circuits=3, num_shots=100, method=1,
    # App-specific args
    input_value=None,
    api=None,
):
    """Create Hidden Shift benchmark circuits.
    ...
    kernel = qedc_get_kernel("hs_kernel", api=api)
...
def analyze_and_print_result(qc, result, num_qubits, num_shots, s_int=None):
    ...
    # Expected result is always the secret_int measured with 100% probability
...
def run_circuits(all_qcs,
    ...
    """
    qedc_get_kernel("hs_kernel", api=api)
...
import argparse
def get_args():
    parser = argparse.ArgumentParser(description="Hidden Shift Benchmark")
