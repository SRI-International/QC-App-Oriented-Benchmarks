"""
Variational Quantum Eigensolver Benchmark Program - Fire Opal
"""
...
def ClusterOperatorCircuit(pauli_op, excitationIndex):
    ...
    global CO_
    if CO_ is None or qc.num_qubits <= 4:
...
def readPauliExcitation(norb, circuit_id=0):
    ...
    filename = os.path.join(os.path.dirname(__file__), f"../qiskit/ansatzes/{norb}_qubit_{circuit_id}.txt")
...
def ReadHamiltonian(nqubit):
    ...
    filename = os.path.join(
        ...
        f"../qiskit/Hamiltonians/{nqubit}_qubit.txt"
    )
    with open(filename) as f:
        data = f.read()
...
def analyze_and_print_result(qc, result, num_qubits, references):
...
def run(
    ...
    print("Variational Quantum Eigensolver Benchmark Program - Fire Opal")
    ...
    def execution_handler(qc, result, num_qubits, type):
        ...
        fidelity = analyze_and_print_result(qc, result, num_qubits, references)
    ...
    print(Hf_ if Hf_ is not None else " ... too large!")
    print("\nCluster Operator Example 'Cluster Op' =")
    print(CO_ if CO_ is not None else " ... too large!")

    # Plot metrics for all circuit sizes
    metrics.plot_metrics(f"Benchmark Results - VQE Simulation ({method}) - Fire Opal")
